...
import { FaRegUserCircle, FaBars, FaPowerOff } from "react-icons/fa";
...
const CustomerNavbar = () => {
  ...
  return (
    <header
      className={`${styles.container} container text-white flex-wrap  sm:py-4 py-2  sm:px-6 px-3 flex justify-between items-center`}
    >
      {/* Logo */}
          ...
          {showWallet && (
            <ul className="list-none  text-black absolute top-[60px] shadow-xl  w-64 px-2 bg-white z-[100] right-0">
              <li className="  flex  justify-between w-full hover:bg-[#F4F4F4] items-center gap-x-2 px-2 py-2  border-b-2  border-b-gray-300 cursor-pointer">
                ...
                <p className="flex items-center text-green font-bold "><MdOutlineCurrencyRupee/>2345</p>
                
              </li>
              {profileLinks.map((link)=>(
                 <li key={link.id} className="flex hover:bg-[#F4F4F4] items-center gap-x-2 px-2 py-3   border-b border-b-gray-300 cursor-pointer">
                 <div className="icon">
                   {link.icon}
                 </div>
                 <p>{link.title}</p>
